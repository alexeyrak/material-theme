$.fn.filterDirective = function() {

    var timeout = null;
    var $filter = $(this).find('.filters-block');

    function initHeight() {
        setTimeout(function() {
            $filter.css({
                'min-height' : $filter.find('.filter-container').outerHeight()
            })
        });
    }
    ...
    function filterTimeout() {
        ...
        timeout = setTimeout(function() {
            if($filter.hasClass('filters-block-hidden')) {
                $filter.css('overflow', '');
            }
            else {
    ...
    $(this).find('.toggle-filter').click(function() {

        $filter.css('overflow', 'hidden');
        $filter.toggleClass('filters-block-hidden');
...
$.fn.fixedHeader = function() {
    ...
    $(this).each(function() {
        ...
        var $fHeader = $header.clone();
        var tClass   = $(this).attr('class');
        var width    = $(this).outerWidth();
        var $table   = $('<table/>').addClass(tClass).css({
            display: 'none',
            position: 'fixed',
            top: 0,
        ...
        function init() {
            $table.empty();
            $fHeader.find('th').each(function(i) {
                var w = $th.eq(i).outerWidth();
                $(this).css({
                    width : w,
                    'min-width' : w
                })
            ...
            $table.append($fHeader);
            $table = $table.insertBefore($this);
            reinit();
        ...
        function toTop() {
            ...
            var bottom = top + $this.height() - $table.height();
